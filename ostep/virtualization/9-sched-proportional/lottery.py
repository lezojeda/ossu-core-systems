...
print(f"Job list argument: {options.jlist}")
print(f"Total jobs: {options.jobs}")
print(f"Maximum job length: {options.maxlen}")
print(f"Maximum ticket count: {options.maxticket}")
print(f"Quantum size: {options.quantum}")
print(f"Random seed: {options.seed}\n")

print("Listing jobs with their durations and ticket allocations: ")
...
if options.jlist == '':
    for jobnum in range(0,options.jobs):
        ...
        print(f"  Job {jobnum} | duration: {runtime} | tickets: {tickets}")
else:
    jobnum = 0
    for entry in options.jlist.split(','):
        ...
        jobnum += 1
    for job in joblist:
        print(f"  Job {job[0]} | duration: {job[1]} | tickets: {job[2]}")
print("\n")

if not options.solve:
    print("Random numbers required for simulation:")
